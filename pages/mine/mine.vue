<script setup>
	...
	const onPopupClose = () => {
		popupService.value.close();
	}
	const onMakePhone = () => {
		uni.makePhoneCall({
			phoneNumber: '17398888669',
			complete: () => {
				onPopupClose('service');
			}
		});
	}
...
<template>
	...
	<view class="page">
		...
		<uni-popup ref="popupService" type="center">
			<v-popup-dialog message="客服热线：17398888669" sureButtonText="立即拨打" @cancel="onPopupClose('service')" @sure="onMakePhone" sureButtonBgColor="#42b983" />
		</uni-popup>
		<!-- 内容 end -->
	</view>
</template>
